...
self="$(readlink -m "$0")"
cd "$here"
target="/var/www/html/cso1-s2022/website/"
remote="portal.cs.virginia.edu:/u/lat7h/public_html/CSO1/S2022/"
gsite="kytos:/var/www/html/cso1-s2022/"
...
function upfile() {
    ...
    if [ "${src%.md}" != "$src" ]; then
        ...
        || [ "$dst" -ot "${here}env.py" ]; then upfile_inner "$src" "$dst"; return 0; fi
    fi
}
...
|| [ Graham-links.yaml -nt schedule.html ] \
...
git pull
git push
rsync -avu --delete-delay "live/" "${gsite}quizzes/questions"
scp *.json "${gsite}quizzes/"
