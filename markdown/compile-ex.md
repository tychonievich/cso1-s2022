...
Note the optimization: the code now just says `%al` is `1` which is what the previous math (`(1+2)>>1`) evaluates to;
the floating-point equivalent of that is placed in a memory location to be loaded by the `movsd` command.
...
Note that `xmm0` now has the address of the floating-point literal,
`edi` now has the address of the string literal,
and `callq` now goes to the address of the statically-linked `printf` function.
:::
